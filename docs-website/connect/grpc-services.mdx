...
Services expose native gRPC or ConnectRPC endpoints that the router calls over the network and integrate as subgraphs in your federated graph. They are deployed, scaled, monitored, and released independently. See [ConnectRPC transport configuration](/router/gRPC/grpc-services#connectrpc-transport) for protocol selection, URL requirements, and migration guidance.
