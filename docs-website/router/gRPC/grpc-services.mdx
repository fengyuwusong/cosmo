...
The Control Plane routing URL for a gRPC service remains a native gRPC resolver target in this first release. ConnectRPC Router deployments must keep local HTTP(S) routing URL overrides. Override keys use the exact subgraph name from each router config, so feature subgraphs with generated names require their own entries.
...
The Router rejects enabled per-subgraph `tls.client_grpc` settings for remote ConnectRPC subgraphs and warns when global `tls.client_grpc` settings are present. Move those settings to `tls.client`; Router Plugins continue to use `tls.client_grpc`.
...
2. Add HTTP(S) routing URL overrides for every remote gRPC service in every base and feature graph config. Use each config's exact subgraph name.
3. In each Router deployment, atomically switch `grpc_protocol.default_protocol` to `connectrpc` and deploy the overrides. Move custom CA or mTLS settings from `tls.client_grpc` to `tls.client` in the same change.
4. Verify every deployment, then keep the local overrides while ConnectRPC is enabled. The Control Plane routing URL remains the native gRPC resolver target.

Rollback by switching `grpc_protocol` back to `grpc` and removing the HTTP(S) overrides in the same deployment. The Router validates every base and feature graph before it starts, so a missing or incompatible override intentionally prevents startup instead of allowing a partially migrated federation.
