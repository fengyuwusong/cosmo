...
Remote gRPC service subgraphs use native gRPC by default. To call all remote gRPC service subgraphs with ConnectRPC instead, configure `grpc_protocol`:
...
grpc_protocol:
  ...
  connectrpc_encoding: proto # proto or json
```

| Environment Variable                     | YAML                              | Required               | Description                                                                                          | Default Value |
| ---------------------------------------- | --------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------------------- | ------------- |
| GRPC_PROTOCOL_DEFAULT_PROTOCOL           | grpc_protocol.default_protocol    | <Icon icon="square" /> | Protocol for every remote gRPC service subgraph. One of `grpc` or `connectrpc`.                       | grpc          |
| GRPC_PROTOCOL_CONNECTRPC_ENCODING        | grpc_protocol.connectrpc_encoding | <Icon icon="square" /> | Global ConnectRPC request encoding. One of `proto` or `json`; used only when ConnectRPC is selected. | proto         |

This first release selects one protocol and one ConnectRPC encoding for all remote gRPC service subgraphs. Router Plugin subgraphs always use native gRPC. Native gRPC routing URLs use the existing gRPC resolver schemes such as `dns:///service:443`; ConnectRPC routing URLs must be absolute `http://` or `https://` URLs. Base paths and trailing slashes are supported, but user information, query strings, and fragments are rejected.

<Note>
  `grpc_protocol` selects how the Router calls remote gRPC subgraphs. It is unrelated to [`connect_rpc`](/connect-rpc/produce-configure-router), which configures the Router's own inbound ConnectRPC server.
</Note>

See [gRPC Services](/router/gRPC/grpc-services#connectrpc-transport) for transport behavior, TLS settings, and migration guidance.
...
The `tls.client_grpc` settings below apply to native gRPC subgraphs and Router Plugins. Native gRPC TLS must be explicitly enabled with `enabled: true`.

ConnectRPC subgraphs use the HTTP transport and therefore inherit `tls.client` settings instead. A deployment moving from native gRPC to ConnectRPC must move any custom CA or mTLS configuration from `tls.client_grpc` to `tls.client` as part of the same rollout.

When ConnectRPC is selected, the Router rejects enabled `tls.client_grpc.subgraphs` entries that match remote ConnectRPC subgraphs. An enabled `tls.client_grpc.all` produces a startup warning because it is ignored by remote ConnectRPC subgraphs but may still be used by Router Plugins.
...
ConnectRPC subgraph calls use the Router's HTTP transport and inherit these rules, including request timeouts, query retries, circuit breakers, proxy settings, and connection pooling. Native gRPC subgraphs do not use the HTTP retry and timeout layer.
