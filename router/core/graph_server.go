...
func (s *graphServer) buildGraphMux(
	...
	* Initialize base attributes from headers and other sources
	...
	connectTransports, err := grpcprotocol.BuildConnectTransports(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to build connect transports: %w", err)
	}

	if err := s.setupConnector(s.graphServerCtx, opts.EngineConfig, opts.ConfigSubgraphs, telemetryAttExpressions, tracingAttExpressions, connectTransports); err != nil {
